...
void GeneralGame::set_valores() {
	...
	Window.create(sf::VideoMode(1280, 720), "Piratas do CPP",
	...
	// Setvalues for icon
	Icon.loadFromFile("../assets/Ico.png");
	...
	// Setvalues for Menu
	font->loadFromFile("../assets/BattlesbridgeDemo-AL126.ttf");
	Numberfont->loadFromFile("../assets/Numbers.ttf");

	image.loadFromFile("../assets/part1.png");
	bg.setTexture(image);
	pos_mouse = { 0, 0 };
	mouse_coord = { 0, 0 };

	// Setvalues for Configs
	VolumeImage.loadFromFile("../assets/Ciricle.png");
	VolumeBg.setScale(0.5, 0.5);
	VolumeBg.setPosition(0, 300);
	VolumeBg.setColor(sf::Color::Magenta);
	music->openFromFile(
			"../assets/Pirates of the Caribbean The Curse of the Black Pear.ogg");
	music->setVolume(10);
	...
	Agua.loadFromFile("../assets/EfeitosSonoros/splash1.wav");
	Tiro.loadFromFile("../assets/EfeitosSonoros/tiro.wav");
	Fogo.loadFromFile("../assets/EfeitosSonoros/Fire.ogg");
	agua.setBuffer(Agua);
	agua.setVolume(100);
	fogo.setBuffer(Fogo);
	fogo.setVolume(100);
...
void GeneralGame::Configs() {
	...
	image.loadFromFile("../assets/part2.png");
	...
	if (SelectConfig.getGlobalBounds().contains(mouse_coord)) {
		...
		SubtextTitulo.setFillColor(sf::Color::Magenta);
		NumericKeyBoard = 2;
	} else if (!SelectConfig.getGlobalBounds().contains(mouse_coord)
			&& NumericKeyBoard == 0) {
		SubtextTitulo.setFillColor(sf::Color::Cyan);
...
void GeneralGame::Levels() {
	...
	image.loadFromFile("../assets/part5.png");
	SelctMouse.setSize(sf::Vector2f(206, 52));
	...
	} else if (!SelectPlayLevels.getGlobalBounds().contains(mouse_coord)
			&& NumericKeyBoard == 0) {
		text2.setFillColor(sf::Color::Cyan);
